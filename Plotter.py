...
# This program creates an image that visualizes a given log file.
...
target_file = ap.parse_args()
yearly_quota = ap.parse_args().Quota
...
copy = os.path.join(current_path, "copy.log-example")
test = os.path.join(current_path, "testinput.log-example")
...
for row in Data:
    if translate_date_to_sec(row[11]) > 0:
        ...
        plot_array[x, 1] = (row['AllocCPUS'] * (end_t - start_t).seconds / 3600)
        x = x + 1
...
# reading in testdata
second_data = np.loadtxt(test, dtype='S256', delimiter='|', skiprows=0, usecols=(1, 14, 26, 27))
...
for row in second_data:
    file.write(second_data[z][0].decode('UTF-8') + " " + second_data[z][2].decode('UTF-8') + " " +
               second_data[z][1].decode('UTF-8') + "\n")
    z = z + 1
file.close()
...
fig.savefig(target_file.Output[0], dpi=500)
