...
import datetime # Used to convert our asci dates into unix-seconds
...
# translate_date_to_sec receives a date and returns the date in unix-time, if it's a valid date,
...
tresholds = [0.7, 1.1, 1.5]  # If the usage this month is below this times the quota,
colors = ['lightblue', "#008000", '#ffa500']  # the quota will be colored in
maximum = '#ff0000'  # if the usage is above the highest times the quota, the quota will be colored in this color
...
def colorisation (value, comp):
    ...
    """
    if value/comp < tresholds[0]:
        return colors[0]
    if value / comp < tresholds[1]:
        return colors[1]
    if value / comp < tresholds[2]:
        return colors[2]
    else:
        return maximum
...
# loads the file specified in original/Source, noteworth are 'ReqCPUS', 'Start' and 'End' (3,26,27)
Data = np.loadtxt(original, dtype=data_type, delimiter='|', skiprows=0, usecols=(1, 3, 4, 5, 6, 7, 8, 9, 12, 13, 26, 27))


plot_array = (np.zeros((Data.size, 3)))
x = 0
time_accumulator = 0
...
for row in Data:
    if translate_date_to_sec(row[11]) > 0:  # row[11] contains the end
        ...
        y_start1 = min(y_start1, (row['AllocCPUS'] * (end_t - start_t).seconds)/3600)  # calculate the lowest
        y_end1 = max(y_end1, (row['AllocCPUS'] * (end_t - start_t).seconds) / 3600)
        plot_array[x, 0] = end_t.timestamp()
        plot_array[x, 1] = (row['AllocCPUS'] * (end_t - start_t).seconds / 3600)  # writes the duration into the array
        x = x + 1
...
for itera in range(0, int(number_of_instances)):
    for i in range(0, np.size(tmp_x)):
        if tmp_x[i].timestamp() <= tmp_x2[itera * 3]:
            ...
            break
    tmp_y2[itera * 3] = temporary
    tmp_y2[itera * 3 + 1] = temporary + partial_quota
    tmp_y2[itera * 3 + 2] = temporary + partial_quota
    tmp_x2[itera * 3 + 2] = tmp_x2[itera * 3 + 2] + seconds_per_instance
tmp_x3 = tmp_x
...
for iterator in range(0, int(number_of_instances - 1)):
