...
if mode == 4:
    for p in range(start, end+1):
        results.append(a)
        #funtodraw = a
plt.plot(range(start, end+1), results, color='grey', alpha=0.7)
xval = []
means = []
...
if fexists:
    #print("fexists")
    for i in range(max(int(focus-0.05*(end-start)),int(start)),min(int(focus+0.05*(end-start)), end)):
        # print(i)
        f.append(results[i-start])
        x2.append(i+1)
        #t+=str(i)
