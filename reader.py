...
textlist = []
tablelist=[]
tablelist.append(";".join(["Job nr.","Account (Projectname)","efficiency (%)","totalCPU (hours)", 'ElapsedRaw (Seconds)',"number of CPUs:", 'number of nodes:',"Memory:","Corehours","Parameter: efficiency >= "+str(mini*100)+"%, efficiency >= "+str(maxi*100)+"%.","Runtime >= "+str((LowerLimit/600)//10)+" minutes"]))
textlist.append("Parameter: efficiency >= "+str(mini*100)+"%, efficiency >= "+str(maxi*100)+"%.")
textlist.append("Runtime >= "+str((LowerLimit/6)//10)+" minutes")
for i in range(len(Data)):
    if translate_date_to_sec(Data[i]['End']) > 0 and '.' not in str(Data[i]['JobID']) and filter_ in str(Data[i]['Account'])and str(Data[i]['End'])[2:-1] >= startpoint and Data[i]["ElapsedRaw"] > LowerLimit:
        ...
        if efficiency <= maxi and efficiency >= mini:
            ...
            if efficiency <= 0.01:
                print(Data[i])
            seqofdata = [str(id), str(acc), str(int(abs(efficiency*100000))/1000), str((translate_time_to_sec(formated)//36)/100), str((Data[i]['ElapsedRaw']//36)/100), str(Data[i]['NCPUS']), str(Data[i]['NNodes']), (str(Data[i]['ReqMem'])[2:-1]), str((int(Data[i]['CPUTimeRAW']//36)/100))]
            excl = ";".join(seqofdata)
            s = "Job nr. " + str(id).ljust(8) + " (account = " + acc + ") has the efficiency " + str(int(abs(efficiency*1000))/10).ljust(5)+"% with a totalCPU of "\
                + str((translate_time_to_sec(formated)//360)/10).ljust(8) + " hours. Runtime in hours:"+str((Data[i]['ElapsedRaw']//360)/10).ljust(6)+" number of CPUs:"+str(Data[i]['NCPUS']).ljust(4)+' number of nodes:'\
                +str(Data[i]['NNodes']).ljust(4)+". Memory:"+(str(Data[i]['ReqMem'])[2:-1]).ljust(9)+". Corehours:"+str(int(Data[i]['CPUTimeRAW']//360)/10)
            textlist.append(s)
            tablelist.append(excl)
